...
function initChatbot(module) {
  ...
  appendBotMessage('Hi, I\'m CloudSentinel AI. Ask me about your detected risks, remediation steps, or what to prioritize first.');
}
...
async function sendChat() {
  ...
  try {
    ...
    removeTyping(typingId);
    appendBotMessage(`Error: ${escHtml(err.message)}. Please ensure the API is reachable.`);
  }
}
...
function appendBotMessage(text) {
  const msgs = document.getElementById('chatbot-messages');
  if (!msgs) return;
  msgs.insertAdjacentHTML('beforeend', `<div class="chat-msg bot"><div class="chat-avatar">AI</div><div class="chat-bubble">${text}</div></div>`);
  msgs.scrollTop = msgs.scrollHeight;
}
...
function appendTyping() {
  ...
  msgs?.insertAdjacentHTML('beforeend',
    `<div class="chat-msg bot" id="${id}"><div class="chat-avatar">AI</div><div class="chat-typing"><span></span><span></span><span></span></div></div>`);
