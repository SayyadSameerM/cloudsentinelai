...
def main() -> None:
    ...
    log.info("")
    log.info("Set the following in your frontend HTML files:")
    log.info("  window.ENV_API_URL           = '%s'", api_url)
    log.info("  window.ENV_COGNITO_POOL_ID   = '%s'", pool_id)
    log.info("  window.ENV_COGNITO_CLIENT_ID = '%s'", client_id)
    log.info("  window.ENV_REGION            = '%s'", region)
    log.info("=" * 60)
